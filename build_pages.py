...
<footer>
    ...
    <div class="footer-copy">&copy; 2026 Party Maker. All rights reserved. | <a href="mailto:info@partymaker.cn">info@partymaker.cn</a> | <a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener">浙ICP备2026022639号-1</a></div>
</footer>
...
def generate_blog_posts(blog_json_path, output_dir, css_path="/style.css"):
    ...
    for post in posts:
        ...
        # Build body HTML + generate ToC from h2 headings
        body_parts = []
        toc_items = []
        toc_counter = 0
        for block in post.get('body', []):
            ...
            if t == 'h2':
                ...
                toc_counter += 1
                toc_items.append(f'<li><a href="#{anchor}">{content}</a></li>')
        ...
        toc_html = ''
        if len(toc_items) > 2:
            toc_html = '<nav class="blog-toc"><h3>In This Article</h3><ul>' + '\n'.join(toc_items) + '</ul></nav>'
