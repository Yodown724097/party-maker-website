...
function buildCsv(contact, cart, piNo, total) {
  ...
  lines.push('');
  lines.push('No.,SKU,Image,Product Name,USD Price,Qty,Amount,Cost(CNY),Unit Size');
  ...
  cart.forEach((item, i) => {
    ...
    lines.push(`${i+1},"${sku}","${imageUrl}","${name}",${price},${qty},${(qty*price).toFixed(2)},${costPrice > 0 ? costPrice.toFixed(2) : ''},${unitSize}`);
  });
